<template>
    <tr class="data-row">
        <td class="fixed cursor-pointer p-0"  @click="toggleTask(projectObjectData)">
            <div class="d-flex align-items-center">
                <div class="d-flex align-items-center">
                    <img v-if="projectObjectData.status !== 'close' && (projectObjectData.deletedStatusKey == 0 || projectObjectData.deletedStatusKey == undefined)" alt="" :src="table_arrow" class="taable_arrow mr-10px" :class="{'row-open': rowOpen}"/>
        ...
        <td v-for="(colName, index) in themodelValue" :key="index"  v-bind:class="[{ 'weekoff': colName.day == 'Sat' || colName.day == 'Sun', 'current_date': colName.fullDate == colName.currentDate}]">
            <div class="inner_div displayRate d-flex" >
                <span class="total__project-title pt-5px position-re bg-transparent" :class="{'cursor-pointer': Object.keys(projectObjectData.projectFinalLogs).includes(colName.fullDate) }" @click="openLogSetailSideBar(Object.keys(projectObjectData.projectFinalLogs).includes(colName.fullDate),colName,'project')" >
            ...
            <td :colspan="tableStyle.colspanCount" class="p-0">
                <table class="table" v-bind:class="[{ 'isWeeked': activeWeekObject.isWeeked,'isOneday': activeWeekObject.isOneday,
                    ...
                    <tr v-for="taskObject in projectObjectData.activitiesArray" :key="taskObject.id">
                        ...
                        <td v-for="(colName, index) in themodelValue" :key="index"  v-bind:class="[{ 'weekoff': colName.day == 'Sat' || colName.day == 'Sun', 'current_date': colName.fullDate == colName.currentDate}]">
                            <div class="inner_div displayRate d-flex">
                                <span class="total__project-title pt-5px position-re bg-transparent" :class="{'cursor-pointer': Object.keys(taskObject.taskFinalLogs).includes(colName.fullDate) }"  @click="openLogSetailSideBar(Object.keys(taskObject.taskFinalLogs).includes(colName.fullDate),colName,'task',taskObject)">
                                     {{ Object.keys(taskObject.taskFinalLogs).includes(colName.fullDate) ? convertedTimeString(taskObject.taskFinalLogs[colName.fullDate],'update') : ''}}
...
<script setup>
    ...
    provide("isRouteRequired", ref(false));
...
<style scoped>
td .inner_div span:last-child{background-color: transparent;}
